...
function Drawer({ open, onClose, navigate, location, order, setOrder, nav, setNav }) {
  ...
  const togglePin = (path) => {
    if (nav.includes(path)) {
      setNav(nav.filter(p => p !== path))
    } else if (nav.length >= NAV_SLOTS) {
      setNotice('Tab bar is full — unpin one first')
    } else {
      setNav([...nav, path])
    }
  }
  ...
  // long-press to pick up, then drag to reorder
  ...
  const onTouchMove = (e) => {
    if (dragIdx === null) {
      // moved before the press registered — treat it as a scroll, not a drag
      if (Math.abs(e.touches[0].clientY - startY.current) > 8) clearTimeout(pressTimer.current)
      return
    }
    const t = e.touches[0]
    ...
    if (row) {
      const over = Number(row.getAttribute('data-row'))
      if (over !== dragIdx) moveTo(dragIdx, over)
    }
  }
  const onTouchEnd = () => { clearTimeout(pressTimer.current); setDragIdx(null) }
  ...
  return (
    <>
      ...
      <div className="drawer-panel"
        ...
        <div ref={listRef} style={{ flex: 1, paddingBottom: 12, touchAction: dragIdx !== null ? 'none' : 'auto' }}>
          {order.map((path, i) => {
            const d = byPath(path)
            if (!d) return null
            const isActive = location.pathname === path
            const pinned = nav.includes(path)
            const isDragging = dragIdx === i
            return (
              <div key={path} data-row={i}
                draggable
                onDragStart={() => setDragIdx(i)}
                onDragEnter={() => dragIdx !== null && moveTo(dragIdx, i)}
                onDragEnd={() => setDragIdx(null)}
                onDragOver={e => e.preventDefault()}
                onTouchStart={e => onTouchStart(e, i)}
                onTouchMove={onTouchMove}
                onTouchEnd={onTouchEnd}
                onClick={() => dragIdx === null && go(path)}
                style={{
                  display: 'flex', alignItems: 'center', gap: 13,
                  padding: '11px 20px', cursor: 'pointer',
                  background: isDragging ? 'var(--bg-card2)' : isActive ? 'var(--accent-dim)' : 'transparent',
                ...
                <div onClick={e => { e.stopPropagation(); togglePin(path) }}
