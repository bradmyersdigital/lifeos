...
function SectorModal({ sector, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    else await supabase.from('sectors').insert({ name: name.trim(), icon, color })
    setSaving(false); onSaved(); onClose()
  }
  const handleDelete = async () => {
    if (!window.confirm(`Delete "${sector.name}"?`)) return; setDeleting(true)
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <div className="field"><div className="field-label">Sector name</div><input type="text" placeholder="e.g. Business, Health..." value={name} onChange={e => setName(e.target.value)} /></div>
        <div className="field">
  ...
  const TaskRow = ({ task }) => {
    ...
    return (
      <div onClick={() => onEditTask(task)} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 14px', background: '#161618', border: '1px solid #242428', borderRadius: 12, marginBottom: 6, opacity: task.completed ? 0.4 : 1, cursor: 'pointer' }}>
        <div style={{ width: 20, height: 20, borderRadius: '50%', border: `1.5px solid ${task.completed ? '#d4520f' : '#333'}`, background: task.completed ? '#d4520f' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} onClick={e => { e.stopPropagation(); toggleTask(task) }}>
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#555', marginTop: 1 }}>{tasks.length} tasks · {notes.length} notes · {projects.length} projects</div>
        </div>
      </div>
      ...
      {tab === 'tasks' && (
        <div>
          {tasks.length === 0 && <div style={{ textAlign: 'center', padding: '30px', color: '#444', fontSize: 13 }}>No tasks in this sector</div>}
      ...
      {tab === 'notes' && (
        <div>
          {notes.length === 0 && <div style={{ textAlign: 'center', padding: '30px', color: '#444', fontSize: 13 }}>No notes in this sector</div>}
          {notes.map(note => (
      ...
      {tab === 'projects' && (
        <div>
          {projects.length === 0 && <div style={{ textAlign: 'center', padding: '30px', color: '#444', fontSize: 13 }}>No projects in this sector</div>}
          {projects.map(p => {
            const pt = p.tasks || []
...
export default function Sectors({ onEditTask, onAddTask }) {
  ...
  if (selected) return <SectorDetail sector={selected} onBack={() => setSelected(null)} onEditTask={onEditTask} onAddTask={onAddTask} />
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12 }}>
        {sectors.map(s => (
          <div key={s.id} onClick={() => setSelected(s)} style={{ background: '#161618', border: '1px solid #242428', borderRadius: 16, padding: 20, cursor: 'pointer', position: 'relative' }}>
        ...
        {sectors.length === 0 && <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '40px', color: '#444', fontSize: 14 }}>No sectors yet — tap New sector</div>}
      </div>
