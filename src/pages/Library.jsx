...
export default function Library() {
  const [categories, setCategories] = useState(['Inspiration','Progress','Family','Travel','Work','Other'])
  ...
  const changeCategory = (itemId, newCat) => {
    setItems(prev => prev.map(i => i.id === itemId ? { ...i, category: newCat } : i))
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div onClick={() => fileRef.current?.click()} className="btn-task action-btn" style={{ gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4, marginBottom: 16, WebkitOverflowScrolling: 'touch' }}>
        ...
        <div onClick={() => setShowAddCategory(!showAddCategory)}
          style={{ padding: '6px 14px', borderRadius: 20, fontSize: 12, fontWeight: 500, cursor: 'pointer', border: '1px dashed #242428', whiteSpace: 'nowrap', color: '#555' }}>
          + New category
        </div>
      </div>
      ...
      {items.length === 0 && (
        <div onClick={() => fileRef.current?.click()} style={{ textAlign: 'center', padding: '60px 20px', color: '#444', fontSize: 14, border: '2px dashed #242428', borderRadius: 16, cursor: 'pointer' }}>
          ...
          <div style={{ fontSize: 12, color: '#333' }}>Supports images and videos</div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
        {filtered.map(item => (
          <div key={item.id} style={{ position: 'relative', borderRadius: 12, overflow: 'hidden', aspectRatio: '1', background: '#161618', cursor: 'pointer' }}>
            {item.type === 'video'
              ? <video src={item.url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onClick={() => setLightbox(item)} />
              : <img src={item.url} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onClick={() => setLightbox(item)} />
            }
      ...
      {lightbox && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', zIndex: 300, display: 'flex', flexDirection: 'column' }} onClick={() => setLightbox(null)}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px' }} onClick={e => e.stopPropagation()}>
            <select value={lightbox.category} onChange={e => { changeCategory(lightbox.id, e.target.value); setLightbox(prev => ({ ...prev, category: e.target.value })) }}
