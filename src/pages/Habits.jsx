...
function HabitModal({ habit, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    if (isEdit) await supabase.from('habits').update({ name: name.trim(), icon }).eq('id', habit.id)
    else await supabase.from('habits').insert({ name: name.trim(), icon })
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <div className="field"><div className="field-label">Habit name</div><input type="text" placeholder="e.g. Workout, Read, Meditate..." value={name} onChange={e => setName(e.target.value)} /></div>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <div onClick={() => setHabitModal('new')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#1e1208', border: '1px solid #7a3410', borderRadius: 10, padding: '7px 12px', cursor: 'pointer', fontSize: 13, color: '#d4520f', fontWeight: 500 }}>
            + Add
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 20 }}>
        {[['Today', `${doneToday}/${habits.length}`, '#e8e6e1', 'completed'],['Best streak', Math.max(0,...habits.map(h=>getStreak(h.id))), '#d4520f','days'],['This week', habits.length ? Math.round(logs.filter(l=>{const d=new Date(l.completed_date),ws=new Date(today);ws.setDate(today.getDate()-todayIdx);return d>=ws&&d<=today}).length/(habits.length*7)*100)+'%':'0%','#e8e6e1','completion']].map(([label,val,color,sub])=>(
      ...
      {view === 'today' && (
        <div>
          <div style={{ fontSize: 12, color: '#444', marginBottom: 12 }}>Hold to reorder · tap day dots to edit past days</div>
          {habits.map((habit, idx) => {
            ...
            return (
              <div key={habit.id}
                ...
                style={{ background: '#161618', border: `1px solid ${done ? '#1a3a1a' : '#242428'}`, borderRadius: 14, padding: 16, marginBottom: 10, cursor: 'grab', userSelect: 'none' }}
              >
                  ...
                  {DAY_LABELS.map((label, i) => {
                    ...
                    return (
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                        <div onClick={() => (isPast || isToday) && toggleLog(habit.id, date)}
                          style={{ width: 28, height: 28, borderRadius: '50%', background: dayDone ? (isToday ? '#d4520f' : '#16a34a') : isPast ? '#1a1a1a' : '#1e1e24', border: `1px solid ${isToday && !dayDone ? '#d4520f' : dayDone ? 'transparent' : '#2a2a30'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: isPast || isToday ? 'pointer' : 'default', transition: 'all 0.15s' }}>
