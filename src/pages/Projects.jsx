        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {IMPORTANCE.map(imp => {
              ...
              return <div key={imp} onClick={() => setImportance(imp)} style={{ flex: 1, padding: '7px 4px', borderRadius: 9, textAlign: 'center', fontSize: 11, fontWeight: 500, cursor: 'pointer', background: active ? s.bg : 'var(--bg)', border: `1px solid ${active ? s.border : 'var(--border)'}`, color: active ? s.color : '#555' }}>{imp}</div>
            })}
          </div>
        </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
        <div onClick={onBack} style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--bg-card)', border: '1px solid #242428', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 18, color: 'var(--text-muted)' }}>‹</div>
        ...
        <div onClick={() => setEditModal(true)} style={{ width: 34, height: 34, borderRadius: 10, background: 'var(--bg-card)', border: '1px solid #242428', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
      ...
      {project.description && <div style={{ fontSize: 14, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.5, padding: '12px 14px', background: 'var(--bg-card)', borderRadius: 12, border: '1px solid #242428' }}>{project.description}</div>}
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:6,marginBottom:20 }}>
        {tasks.map(task => {
          ...
          return (
            <div key={task.id} onClick={() => onEditTask(task)} style={{ display:'flex',alignItems:'center',gap:10,padding:'11px 14px',background:'var(--bg-card)',border:'1px solid #242428',borderRadius:12,opacity:task.completed?0.4:1,cursor:'pointer' }}>
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
        {notes.map(note=>(
          <div key={note.id} onClick={()=>setNoteModal(note)} style={{ background:'var(--bg-card)',border:'1px solid #242428',borderRadius:12,padding:14,cursor:'pointer' }}>
    ...
    <div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:20 }}>
        {[['Active',projects.filter(p=>p.status==='active').length],['Backlog',projects.filter(p=>p.status==='backlog').length],['Done',projects.filter(p=>p.status==='completed').length]].map(([label,val])=>(
          <div key={label} style={{ background:'var(--bg-card)',border:'1px solid #242428',borderRadius:11,padding:12 }}>
      ...
      {filtered.map(project=>{
        ...
        const imp=project.importance?IMP_STYLES[project.importance]:null
        return (
          <div key={project.id} onClick={()=>setSelected(project)} style={{ background:'var(--bg-card)',border:'1px solid #242428',borderRadius:14,padding:16,marginBottom:10,cursor:'pointer' }}>
            <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:10,gap:10 }}>
              <div>
                <div style={{ fontSize:15,fontWeight:500,marginBottom:4 }}>{project.name}</div>
                <div style={{ display:'flex',gap:6,flexWrap:'wrap',alignItems:'center' }}>
                  {project.sector&&<span style={{ fontSize:11,color:'#555',background:'var(--border)',padding:'2px 8px',borderRadius:6 }}>{project.sector}</span>}
