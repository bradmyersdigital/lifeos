    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <div className="field-row">
          <div className="field"><div className="field-label">Sector</div><select value={sector} onChange={e => setSector(e.target.value)}><option value="">Select...</option>{SECTORS.map(s => <option key={s}>{s}</option>)}</select></div>
          <div className="field"><div className="field-label">Status</div><select value={status} onChange={e => setStatus(e.target.value)}><option value="active">Active</option><option value="backlog">Backlog</option><option value="completed">Completed</option></select></div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
          ...
          <button className="btn-primary" style={{ flex: 2 }} onClick={handleSave} disabled={saving}>{saving ? 'Saving…' : isEdit ? 'Save changes' : 'Create project'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-sheet">
        ...
        <div className="field"><div className="field-label">Note</div><textarea placeholder="Add a note for this project..." value={text} onChange={e => setText(e.target.value)} style={{ height: 120 }} /></div>
        <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
...
function ProjectDetail({ project, onBack, onAddTask, onEditTask, onRefresh }) {
  ...
  return (
    <div>
      ...
      {tasks.length === 0 && <div style={{ textAlign: 'center', padding: '20px', color: '#444', fontSize: 13, border: '1px dashed #242428', borderRadius: 12, marginBottom: 18 }}>No tasks yet — add one above</div>}
...
export default function Projects({ onAddTask, onEditTask }) {
  ...
  if (selected) {
    return <ProjectDetail project={selected} onBack={() => setSelected(null)} onAddTask={onAddTask} onEditTask={onEditTask} onRefresh={loadProjects} />
  }
  ...
  return (
    <div>
      ...
      {filtered.length === 0 && <div style={{ textAlign: 'center', padding: '40px 20px', color: '#444', fontSize: 14 }}>No projects yet</div>}
      ...
      {filtered.map(project => {
        ...
        return (
          <div key={project.id} onClick={() => setSelected(project)} style={{ background: '#161618', border: '1px solid #242428', borderRadius: 14, padding: 16, marginBottom: 10, cursor: 'pointer', transition: 'border-color 0.15s' }}>
